  ...
  <div class="shopping-cart">
    ...
    >
      <v-badge
        ...
        :color="isFloating && cartService.getItemCount() > 0 ? 'white' : 'error'"
      ...
      <v-card>
        ...
        <v-card-text style="max-height: 600px;">
          ...
          <div v-if="cartItems.length === 0" class="text-center py-8">
            ...
            <p class="text-grey">Explora nuestros cursos y agrega algunos a tu carrito</p>
            <v-btn
              color="primary"
              @click="$router.push('/courses'); showCartDialog = false"
            >
              Ver Cursos
            </v-btn>
          </div>
          ...
          <div v-else>
              ...
              >
                ...
                <template #append>
                  <div class="text-right">
                    <div v-if="item.originalPrice > item.price" class="text-caption text-decoration-line-through text-grey">
            ...
            <v-card variant="outlined" class="mb-4">
              <v-card-text>
                ...
                <div v-if="appliedCoupon" class="d-flex justify-space-between mb-2 text-success">
                  <span>Descuento:</span>
...
export default {
  ...
  setup(props) {
    ...
    const paymentMethods = [
      { name: 'Tarjeta de Crédito/Débito (Stripe)', value: 'stripe', icon: 'mdi-credit-card' },
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.moodleSSO && result.moodleSSO.available) {
            setTimeout(() => {
              showSnackbar('Puedes acceder directamente a Moodle desde el menú superior', 'info')
            }, 3000)
          }
          ...
          showSnackbar('Debes iniciar sesión para completar la compra', 'warning')
          closeCheckout()
          
          // Emitir evento para abrir dialog de autenticación
          // El componente padre debe manejar esto
          setTimeout(() => {
            showSnackbar('Por favor, inicia sesión y vuelve a intentar la compra', 'info')
          }, 1000)
          
        } else {
          showSnackbar(result.message || 'Error al procesar la compra', 'error')
        }
    ...
    const getCartButtonClass = computed(() => {
      ...
      return `floating-cart-button ${hasItems ? 'cart-has-items' : 'cart-empty'}`
    })
    ...
    const getCartAriaLabel = computed(() => {
      ...
      if (props.isFloating) {
        if (itemCount > 0) {
          return `Abrir carrito de compras - ${itemCount} producto${itemCount > 1 ? 's' : ''} (botón flotante con animación)`
        }
