  ...
  <section class="bg-course">
    <div class="my-courses container">
      <v-container fluid>
        <!-- Header -->
        <div
          class="d-flex justify-content-lg-between justify-content-center flex-lg-row flex-column align-center mb-6"
        >
          <div>
            <h1
              class="txt-cursos display-5 display-lg-4 pt-3 pt-lg-5 text-center text-lg-start"
            >
              Mis Cursos
            </h1>
            <p class="sub-cursos text-center text-lg-start">
        ...
        <div v-else-if="filteredCourses.length === 0" class="text-center py-12">
          ...
          <v-btn color="primary" size="large" @click="$router.push('/catalog')">
            Explorar Cursos
          </v-btn>
        </div>
          ...
          >
            ...
            >
              ...
              <v-card-text class="px-5">
                <!-- Título del curso -->
                <h3 class="txt-title-card fw-bold mb-2">
                  {{ course.title }}
                </h3>

                <!-- Instructor -->
                <div class="d-flex align-center mb-2">
                  <v-icon size="small" class="mr-1 txt-p">mdi-account</v-icon>
                  <span class="text-body-2 text-grey-darken-1 txt-p">
                    {{ course.instructor }}
                  </span>
                </div>
                ...
                <div class="mb-3">
                  <div class="d-flex justify-space-between align-center mb-1">
                    <span class="text-body-2 txt-p">Progreso</span>
                    <span class="text-body-2 font-weight-bold txt-p"
                ...
                <div
                  class="d-flex justify-space-between text-body-2 text-grey-darken-1 mb-2 txt-p"
                >
                  <span>
                ...
                <!-- Última actividad -->
                <div
                  v-if="course.lastActivity"
                  class="text-caption text-grey txt-p"
                >
                  Última actividad: {{ formatDate(course.lastActivity) }}
                </div>
              </v-card-text>

              <v-card-actions class="w-50 m-auto">
                <!-- Botón principal -->
                <v-btn
                ...
                <v-menu>
                  ...
                  <v-list>
                    ...
                    <v-list-item
                      v-if="course.status === 'Completado'"
                      @click="downloadCertificate(course)"
                    >
                      <v-list-item-title
                        >Descargar Certificado</v-list-item-title
                      >
                    </v-list-item>
        ...
        <div v-if="recommendedCourses.length > 0" class="mt-12">
          <h2 class="txt-cursos pb-4 display-6 text-center">
            Cursos Recomendados
          </h2>
          <v-row>
            <v-col
              ...
              >
                ...
                <v-card-text>
                  ...
                  <div class="d-flex justify-space-between align-center">
                    <span class="text-h6 font-weight-bold text-primary txt-p">
...
export default {
  ...
  setup() {
    ...
    const statusOptions = ["No Iniciado", "En Progreso", "Completado"];
    ...
    const filteredCourses = computed(() => {
      ...
      if (filterStatus.value) {
        courses = courses.filter(
          (course) => course.status === filterStatus.value
        );
      }
    ...
    const downloadCertificate = (course) => {
      showSnackbar(
        `Descargando certificado de "${course.title}"...`,
        "success"
      );
    ...
    onMounted(async () => {
      ...
      const recentPurchases = JSON.parse(
        localStorage.getItem("recentPurchases") || "[]"
      );

      if (recentPurchases.length > 0) {
        // Agregar cursos comprados recientemente a la lista
        recentPurchases.forEach((purchase) => {
          const existingCourse = enrolledCourses.value.find(
            (c) => c.id === purchase.id
          );
          if (!existingCourse) {
        ...
        if (recentPurchases.length === 1) {
          showSnackbar(
            `¡Bienvenido a "${recentPurchases[0].title}"! Ya puedes comenzar tu aprendizaje.`,
            "success"
          );
        } else {
          showSnackbar(
            `¡Has adquirido ${recentPurchases.length} cursos nuevos! Ya puedes comenzar tu aprendizaje.`,
            "success"
          );
        }
      }
...
/* Estilos para los parrafos de los cursos */
.sub-cursos {
