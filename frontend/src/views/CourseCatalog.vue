<template>
  <div class="course-catalog">
    <v-container fluid>
      ...
      <header class="d-flex justify-space-between align-center mb-6">
        <div>
          <div class="d-flex align-center mb-2">
            <h1 class="text-h4 font-weight-bold mr-3">Catálogo de Cursos</h1>
            <v-chip
      ...
      <!-- Filtros -->
      <v-card class="mb-6" elevation="1" role="search" aria-label="Filtros de búsqueda de cursos">
        <v-card-text>
          <v-row>
            <v-col cols="12" md="4">
              <v-text-field
          ...
          >
            ...
            >
              ...
              <v-card-text class="pb-2">
                ...
                <!-- Instructor -->
                <div class="d-flex align-center mb-2">
                  <v-icon size="small" class="mr-1" aria-hidden="true">mdi-account</v-icon>
                ...
                <div class="d-flex justify-space-between align-center mb-2">
                  <div class="d-flex align-center">
                    <v-icon size="small" class="mr-1" aria-hidden="true">mdi-clock-outline</v-icon>
                    <span class="text-body-2">Duración: {{ course.duration }}</span>
                  </div>
            ...
            <v-card-actions class="pt-0">
              ...
              <div class="flex-grow-1">
                <div v-if="course.originalPrice > course.price" class="text-caption text-decoration-line-through text-grey">
    ...
    const loadCoursesFromNeekworld = async () => {
      ...
      try {
        ...
        if (result.success && result.courses.length > 0) {
          ...
          showSnackbar(`${result.courses.length} cursos cargados desde Neekworld`, 'success')
        } else {
          throw new Error('No se encontraron cursos en Neekworld')
        }
      } catch (error) {
        console.warn('⚠️ No se pudieron cargar cursos de Neekworld:', error.message)
    ...
    const loadCoursesFromMoodle = async () => {
      ...
      try {
        ...
        if (result.success && result.courses.length > 0) {
          ...
          showSnackbar(`${moodleCourses.length} cursos cargados desde Moodle QA`, 'success')
        } else {
          throw new Error('No se encontraron cursos en Moodle QA')
        }
      } catch (error) {
        console.warn('⚠️ No se pudieron cargar cursos de Moodle, usando cursos de prueba:', error.message)
        dataSource.value = 'mock'
        loadMockCourses()
        showSnackbar('Mostrando cursos de ejemplo (APIs no disponibles)', 'warning')
      } finally {
        loading.value = false
      }
    }
      ...
      allCourses.value = [
        {
          ...
          description: 'Aprende a crear aplicaciones web modernas con Vue.js 3, Vuetify y las mejores prácticas.',
        ...
        },
        {
          ...
          description: 'Domina Python, pandas, numpy y matplotlib para análisis de datos profesional.',
        ...
        },
        {
          ...
          description: 'Estrategias de marketing digital, SEO, SEM, redes sociales y análisis de métricas.',
