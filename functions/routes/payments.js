...
const transbankService = new TransbankPaymentService();
...
router.post('/transbank/create', mockAuth, async (req, res) => {
  try {
    ...
    const result = await transbankService.createTransaction(saleId, amount, returnUrl);
...
router.post('/stripe/create-intent', mockAuth, async (req, res) => {
  try {
    ...
    const paymentIntent = await stripeService.createPaymentIntent(amount, saleId);
